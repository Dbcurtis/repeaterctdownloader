...
class Testupdatetime(unittest.TestCase):
    ...
    def testdelay(self):
        matchtime = time.strptime(
            "03 Jan 2018 11 30 30", "%d %b %Y %H %M %S")
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 56 30", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 59 30", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertTrue(result[1])
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 59 59", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertTrue(result[1])
        self.assertTrue('1 min and 2' in result[1])
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 50 59", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertTrue(result[1])
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 50 00", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertTrue(result[1])
        self.assertTrue('10 min and 2' in result[1])
        ...
        matchtime = time.strptime(
            "03 Jan 2018 23 49 59", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertFalse(result[1])

        matchtime = time.strptime(
            "03 Jan 2018 23 00 00", "%d %b %Y %H %M %S")
        result = updatetime._delay(debug=True, testtime=matchtime)
        curtime = time.localtime(time.time())
        self.assertFalse(result[1])
...
DTMF>"""
        ...
        matchtime = time.strptime(
            "03 Jan 2017 11 30 30", "%d %b %Y %H %M %S")
        cd = updatetime.check_date(_res, sdtpl, matchtime)
        self.assertEqual('N0280103173', cd)

        matchtime = time.strptime(
            "01 Jan 2018 11 30 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_date(_res, sdtpl, matchtime)
        self.assertEqual('N0280101182', cd)

        matchtime = time.strptime(
            "01 Feb 2018 11 30 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_date(_res, sdtpl, matchtime)
...
DTMF"""
        ...
        matchtime = time.strptime(
            "03 Jan 2018 12 23 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        ...
        matchtime = time.strptime(
            "03 Jan 2018 00 30 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertEqual('N02512300', cd)

        matchtime = time.strptime(
            "03 Feb 2018 13 30 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertEqual('N02501301', cd)

        matchtime = time.strptime(
            "03 Jan 2018 23 30 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
...
DTMF"""
        ...
        sttpl = cs.newcmd_dict['stime']
        matchtime = time.strptime(
            "03 Jan 2018 00 23 30", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertFalse(cd)
...
DTMF"""
        ...
        sttpl = cs.newcmd_dict['stime']
        matchtime = time.strptime(
            "03 Jan 2018 00 00 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertFalse(cd)

        matchtime = time.strptime(
            "03 Jan 2018 00 01 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertEqual('N02512010', cd)

        matchtime = time.strptime(
            "03 Jan 2018 23 59 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
...
DTMF"""
        ...
        sttpl = cs.newcmd_dict['stime']
        matchtime = time.strptime(
            "03 Jan 2018 12 00 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertFalse(cd)

        matchtime = time.strptime(
            "03 Jan 2018 12 01 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertEqual('N02512011', cd)

        matchtime = time.strptime(
            "03 Jan 2018 11 59 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
...
DTMF"""
        ...
        self.assertEqual('N02512001', cd)
        matchtime = time.strptime(
            "03 Jan 2018 11 58 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
...
DTMF"""
        ...
        sttpl = cs.newcmd_dict['stime']
        matchtime = time.strptime(
            "03 Jan 2018 01 05 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
        self.assertFalse(cd)
        matchtime = time.strptime(
        ...
        matchtime = time.strptime(
            "03 Jan 2018 01 58 00", "%d %b %Y %H %M %S")
        # time.localtime(time.time()))
        cd = updatetime.check_time(_res, sttpl, matchtime)
    ...
    def testdoit(self):
        ...
        matchtime = time.strptime(
            "03 Jan 2018 12 23 00", "%d %b %Y %H %M %S")
        ...
        matchtime = time.strptime(
            "03 Jan 2018 12 02 00", "%d %b %Y %H %M %S")
        res = stuff.doit(debug_time=matchtime)
...
# still need to test the debug option in the affected code to make sure the time change is not sent to the controler
    ...
    def testprocess_cmdline(self):
        ...
        try:
            ...
            self.assertEqual('RLC-Club Deluxe II v2.15',
                             str(ui.controller_type))
            self.assertFalse(tup[2])
        except Exception as e:
            self.fail(msg="should not have exited {}".format(str(e)))

        try:
            tup = updatetime.process_cmdline(
                ['COM1', ], _testcmdline=['COM1', ])
            ui = tup[0]
        ...
        try:
            ...
            tup = updatetime.process_cmdline(
                ['COM1', ], _testcmdline=['dlx2', 'COM1', '-dbg'])
            ui = tup[0]
            self.assertEqual('COM1', ui.comm_port)
            self.assertEqual('RLC-Club Deluxe II v2.15',
                             str(ui.controller_type))
        ...
        try:
            tup = updatetime.process_cmdline(
                ['COM1', 'COM2', ], _testcmdline=['dlxii', 'COM2', ])
            ui = tup[0]
